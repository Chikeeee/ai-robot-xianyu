...
> **本仓库的增量：闲鱼 7×24 自动值守通道。** 在原有 AI-Robot 之上移植了闲鱼长连接值守能力，
> 并复用本项目的 RAG / 专家层 / 观测 / dashboard，按
> **协议层离线自检 → 只连不回的存活验证 → 影子模式（真连真生成、只落库不发送）→ 值守面板与告警
> → 灰度自动发送** 分期落地（自动发送已上线，真实回复已在平台侧验证送达）。
> 设计见 [`XIANYU-CHANNEL-PLAN.md`](XIANYU-CHANNEL-PLAN.md)，操作手册见
> [`XIANYU-SHADOW-RUNBOOK.md`](XIANYU-SHADOW-RUNBOOK.md)（故障处理 / 灰度台阶 / 回滚三档 / 诊断工具）。
>
> 关键安全设计：**出站闸门**（合规过滤、每会话最小间隔、每分钟·小时·天限速、重复文本窗口、
> 拟人化打字延迟、静默时段、灰度白名单）、**消息级幂等**（跨进程重启，防同一条消息回两次）、
> **会话失效自愈**（平台不关连接只回 401 时自动换 token 重连）、**断线补推**（同步游标落盘）。
>
> ```powershell
> # 一次跑完 14 个离线自检套件（不联网、不连闲鱼、不发任何真实消息）
> .\.venv\Scripts\python.exe scripts\test_xianyu_p0.py
> # 交付验收核对（旧 bot 未被动 / 凭据权限与无明文 / 自动发送状态 / 无重复发送）
> .\.venv\Scripts\python.exe scripts\xianyu_acceptance_check.py
> ```
>
> 凭据只放 `secrets/xianyu_credentials.json`（已 gitignore、权限收紧到仅本账号可读）；
> `.env`、日志与原始帧、SQLite 数据库一律不进仓库。
...
本服务试图把"意图识别 → 工具/RAG 执行 → 流式答复"的客服链路做成**生产可用的完整程序**，而非玩具 Demo：
