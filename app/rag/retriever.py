...
class KnowledgeBase:
    ...
    def ingest_text(self, title: str, text: str) -> int:
        ...
        if docs:
            ...
            # 注意：BM25Index.add_documents 是「整体重建」语义（会覆盖内部 corpus 并重算 BM25），
            # 且 search_bm25 用返回的 index 去索引 self._documents，所以这里必须传全量文档：
            # 只传本次新增的 docs 会导致 ①BM25 只检索最后一个入库文件；②index 与 _documents 错位，
            # 把命中结果标注到完全不相干的块上（多文件入库时实测出现）。
